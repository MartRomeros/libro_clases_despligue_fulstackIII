...
ssh -o StrictHostKeyChecking=no \
  ...
  ubuntu@"$IP_MS_ASISTENCIA" <<REMOTE
...
rm -rf MS_Auth
git clone https://github.com/MartRomeros/MS_Asistencia_Conducta
cd MS_Asistencia_Conducta
...
sudo docker build -t ms_asistencia .

sudo docker rm -f ms_asistencia || true
...
sudo docker ps
sudo docker logs ms_asistencia
